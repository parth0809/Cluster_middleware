            ...
            for (int attempt = 0; attempt < max_retries && !success; ++attempt) {
                ...
                if (!is_transport_failure(response)) {
                    ...
                    cout << "[PRIMARY WARN] Compute node " << target.address << ":" << target.port
                         << " failed delivery. Retrying queue scheduler..." << endl;
                }
            }
...
int main() {
    ...
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
        cerr << "[PRIMARY ERROR] Port 8080 already in use! Is a master or backup already running? Exiting." << endl;
        exit(EXIT_FAILURE);
    }
    
    if (bind(udp_sock, (struct sockaddr *)&udp_addr, sizeof(udp_addr)) < 0) {
        cerr << "[PRIMARY ERROR] Port 8081 already in use! Exiting." << endl;
        exit(EXIT_FAILURE);
    }
